    ...
    return (
        <SafeAreaView className="flex-1 bg-primary">
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="flex-1">
                ...
                <View className="flex-row justify-center items-center mx-6 my-4 p-2 bg-black rounded-full">
                    {['days', 'weeks', 'months'].map((range) => (
                        <TouchableOpacity
                        key={range}
                        onPress={() => setTimeRange(range)}
                        className={`flex-1 py-3 rounded-full ${
                            timeRange === range ? 'bg-slate-400' : 'bg-transparent'
                        }`}
                        >
                ...
                <View className="flex-row justify-center self-center p-2 w-[50%] bg-black rounded-full">
                    {/* Moods */}
                    <TouchableOpacity
                    onPress={() => setViewType('moods')}
                    className={`flex-1 py-3 rounded-full ${
                        viewType === 'moods' ? 'bg-slate-400' : 'bg-transparent'
                    }`}
                    >
                    ...
                    className={`flex-1 py-3 rounded-full ${
                        viewType === 'factors' ? 'bg-slate-400' : 'bg-transparent'
                    }`}
                    >
                ...
                <View className="bg-black rounded-2xl m-4 w-[90%] self-center items-center p-4">
                    <Text className="text-lg text-white font-semibold mb-4">{viewType === 'moods' ? 'Mood History' : 'Factor History'}</Text>
                    <BarChart
                    data={barChartData}
                    barWidth={30}
                    roundedTop
                ...
                <View className="bg-black rounded-2xl m-4 w-[90%] self-center">
                    {/* Title */}
                    <Text className="text-lg text-white font-psemibold m-4">Mood History</Text>
                ...
                <View className="bg-black rounded-2xl m-4 w-[90%] self-center">
                    {/* Title */}
                    <Text className="text-lg text-white font-psemibold m-4">Factors History</Text>
                        ...
                        {Object.entries(factorCounts)
                            ...
                            .map(([key, value]) => (
                                <View key={key} className="mx-4 items-center">
                                    ...
                                    <Text className="text-white mt-2 text-center">{`${key}: ${value}`}</Text>
                                </View>
                        ))}
                    </ScrollView>
                </View>
