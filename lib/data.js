...
export const getJournalData = async (user) => {
    try {
        ...
        // NOTE: if the server response is 404, axios will take it as an error and throw an exception, so it will jump straight to the catch block
...
export const sendJournalData = async (user, entry) => {
    try {
        const res = await axios.post(`http://${localIp}/data/journal`, {...entry, user_id: user.id});
...
// Resources data
export const getResourcesData = async (type, searchQuery) => {
    try {
        const res = await axios.get(`http://${localIp}/data/resources`, {
