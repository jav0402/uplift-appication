...
export const sendQuizResults = async (user, anxiety, depression, other, date) => {
    try {
        // const response = await axios.post(`http://${localIp}/quiz`, {user_id, anxiety, depression, other, date});
        // if (response.status !== 200) throw new Error(response.data);
        console.log(anxiety, depression, other, date)

        return true
    } catch (error) {
        // To change to more specific error handling after endpoint has been made
        console.log(error);
        return false
    }
}
